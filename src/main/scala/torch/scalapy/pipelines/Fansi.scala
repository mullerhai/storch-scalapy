package torch.scalapy.pipelines
 ...
 * the [[Str]]. [[render]] flattens it out into a `java.lang.String`
...
class Str private(private val chars: Array[Char], private val colors: Array[Str.State]) {
  ...
  /**
   * Concatenates two [[Str]]s, preserving the colors in each one and
  ...
  /**
   * Splits an [[Str]] into two sub-strings, preserving the colors in
   * each one.
   *
   * @param index the plain-text index of the point within the [[Str]]
  ...
  /**
   * Returns an [[Str]] which is a substring of this string,
  ...
  /**
   * The plain-text length of this [[Str]], in UTF-16 characters (same
  ...
  /**
   * The plain-text `java.lang.String` represented by this [[Str]],
   * without all the fansi colors or other decorations
   */
  lazy val plainText = new String(chars)

  /**
   * Returns a copy of the colors array backing this `Str`, in case
  ...
  /**
   * Returns a copy of the character array backing this `Str`, in case
   * you want to use it to
   */
  ...
  /**
   * Converts this [[Str]] into a `java.lang.String`, including all
  ...
  /**
   * Overlays the desired color over the specified range of the [[Str]].
   */
...
object Str{
  ...
  /**
   * An [[Str]]'s `color`s array is filled with Long, each representing
  ...
  /**
   * Make the construction of [[Str]]s from `String`s and other
   * `CharSequence`s automatic
   */
  implicit def implicitApply(raw: CharSequence): Str = apply(raw)
  ...
  /**
   * Creates an [[Str]] from a non-fansi `java.lang.String` or other
   * `CharSequence`.
   *
   * Note that this method is implicit, meaning you can pass in a
   * `java.lang.String` anywhere an `Str` is required and it will be
   ...
   */
  def apply(raw: CharSequence, errorMode: ErrorMode = ErrorMode.Throw): Str = {
  ...
  /**
   * Constructs a [[Str]] from an array of characters and an array
  ...
  def fromArrays(chars: Array[Char], colors: Array[Str.State]) = {
    new Str(chars.clone(), colors.clone())
  }
 ...
 * are trying to parse into a [[Str]] contains an Ansi escape not
 * recognized by Fansi as a valid color.
 */
...
sealed trait Attrs{
  ...
  /**
   * Apply these [[Attrs]] to the given [[Str]], making it take effect
   * across the entire length of that string.
   */
  def apply(s: Str) = s.overlay(this, 0, s.length)
  ...
  /**
   * Combine this [[Attrs]] with other [[Attrs]]s, returning one
...
object Attrs{
  ...
  class Multiple private[Attrs] (val resetMask: Long,
    ...
    def ++(other: Attrs) = Attrs(attrs ++ toSeq(other):_*)
  }
...
sealed trait Attr extends Attrs {
   ...
   */
  def ++(other: Attrs): Attrs = Attrs(Array(this) ++ Attrs.toSeq(other):_*)
}
object Attr{
  /**
 ...
 */
case class EscapeAttr (escape: String, resetMask: Long, applyMask: Long)
 ...
 */
case class ResetAttr (resetMask: Long, applyMask: Long)
                                   (implicit sourceName: sourcecode.Name) extends Attr{
  val escapeOpt = None
  val name = sourceName.value
